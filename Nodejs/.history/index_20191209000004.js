...
app.post('/register1/',(req,res,next)=>{
	...
	con.query('INSERT INTO `t_user`(`phoneNum`, `password`, `nickname`) VALUES (?,?,?)',[telephone,password,name], function(err,result,fields)
		{
			con.on('error',function(err){
				console.log('[MySQL ERROR]',err);
				res.json('Resgiter error: ', err);
...
app.get('/getAllCat/', (req, res) => {
    con.query ('SELECT nom,image FROM categorie',function (err,result,status){
		con.on('error',function(err){
			console.log('[MySQL ERROR]',err);
		});
		res.status("success").send({message : 'registrations compelete'})
	});
});
